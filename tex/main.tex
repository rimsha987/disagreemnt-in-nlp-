...
every pooled-label permutation comparison is non-significant. With 123 raters behind each item, a 15.4\%
...
this replication depth, while the tested model generated no demographic variation
distinguishable from its own sampling noise.
\end{abstract}
...
Hu and Collier (2024) measure how much persona conditioning actually moves model
annotations, and Kirk et al.\ (2024) and
...
\item A scope condition, consistent with the present data though not experimentally varied,
under which a demographically biased filter is distributionally mild at high replication depth.
...
Hu and Collier (2024) quantify the effect of persona conditioning directly, reporting that
persona variables account for under 10\% of annotation variance in existing subjective datasets
and that persona prompting yields modest though statistically significant gains. Kirk et al.\ (2024) set out the benefits and
risks of aligning models to individuals and argue for bounds on how far personalisation should
go, while Sorensen et al.\ (2024) formalise pluralistic alignment as the requirement that a
model reflect a distribution of human positions rather than a single aggregate. The methodological gap highlighted by the
...
\label{tab:pool}
\small\begin{tabular}{lr@{\hskip 2em}lr@{\hskip 2em}lr}
\toprule
...
analysis collapses Unsure into No; the resulting mean TVD is 0.01187 compared with 0.01556 for
the original three-way representation, the Spearman correlation between the two per-item series
is $+0.828$, and the substantive conclusion is unchanged.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig1_removal_rate_by_group.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{figures/fig2_tertiles.pdf}
...
% \begin{figure}[htbp]
% \centering
% \includegraphics[width=0.86\textwidth]{figures/fig2c_metric_sensitivity.pdf}
...
The rater-level profiles also support the interpretation that the published exclusion list is
...
narrower range than the complete human pool. These two adjustments run in opposite directions:
selecting the grouping with the largest observed excess inflates the benchmark, while excluding
the two highest-$P(\text{Yes})$ groups deflates it. Their sizes are not known, so they cannot be
netted against one another and neither is treated as cancelling the other. The benchmark is
therefore neither known to overstate nor known to understate the human variation available in
the source data.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.74\textwidth]{figures/fig3_noise_floor.pdf}
...
sampling variation observed under an unchanged persona, which is the case here. The arm
...
sampling floor. The evidence therefore supports the scoped claim that, under these two
...
and 0.044 under v2. Thus, on the matched items, the model is between 5.6 and 6.3 times less likely
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{figures/fig6_between_group_spread.pdf}
...
range; the human condition lies above it in 96.9\% of bootstrap draws (1,939 of 2,000).}
\label{fig:spread}
\end{figure}
...
0.140, with a range of [0.067, 0.267]. No same-persona pair reaches the human mean of 0.524,
so $P(\text{noise} \geq \text{observed})$ is 0.00 for the human condition; that figure compares
the ten same-persona values against the bootstrap mean. Taking the bootstrap distribution as a
whole, 1,939 of 2,000 draws (96.9\%) exceed the largest same-persona value, so the human
contrast is not separated from sampling noise in every draw, only in the large majority of them.
Both LLM conditions, by contrast, fall inside the range produced by unchanged-persona sampling. The model therefore does not demonstrate measurable demographic
...
preserves demographic structure is considered. Unlike the gender observation below, this
...
removal imbalance, so the two observations are consistent. They are not, however, equally strong, and
they are not independent: the removal test was planned while the compression comparison was
selected post-hoc (Section~\ref{sec:removal}), and the same exclusion set generates both, so
...
raised by Fleisig et al.\ (2025). Their concern is relevant to settings with much shallower rater
depth.
...
in Table~\ref{tab:removal}. The two gender findings are affected by multiplicity unequally, and
the distinction is set out in Section~\ref{sec:removal}: the removal imbalance is a planned
comparison that does not survive family-wise correction, while the compression comparison is
post-hoc and takes the full discount. Neither is independent of the other, since the same
exclusion set generates both.
...
pool. In that respect the model was not asked to reproduce the widest available demographic
contrast. This pulls against the selection effect noted in Section~\ref{sec:cells}, where the
grouping was chosen for having the largest observed excess; the two act in opposite directions
and their relative sizes are unknown, so the benchmark is not claimed to be conservative overall.
...
Workshop on Perspectivist Approaches to NLP (NLPerspectives)}, 47--62, Suzhou, China.
...
\emph{Information for a Better World: Shaping the Global Future. 17th International Conference,
iConference 2022}, 425--444. Springer. \url{https://arxiv.org/abs/2112.04030}
...
into No. Under that binary representation the mean TVD between filtered and unfiltered item
distributions is 0.01187, compared with 0.01556 under the three-way representation, and the
Spearman correlation between the two per-item series is $+0.828$. On the secondary metric the
corresponding JSD values are 0.00024 and 0.00055. The substantive conclusion is unchanged under
either metric. The binary figures are smaller because collapsing Unsure into No makes movement
between those two categories invisible, and 6.3\% of all ratings are Unsure. The appendix is intended as a robustness check rather than as the
